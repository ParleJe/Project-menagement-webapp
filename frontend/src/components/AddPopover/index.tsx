...
const AddPopover = ({ show, setShow, idProject }: AddPopoverProps) => {
  ...
  const loadingState = useAppSelector((state) =>
    idProject === undefined ? state.projects.loading : state.hugeTasks.loading
  );
      ...
      <MDBModalDialog centered size="md">
        <MDBModalContent>
          <MDBModalHeader>
            <MDBModalTitle>
              Create New
              <strong className={addType ? 'text-info' : 'text-success'}>
                {' '}
                <u>{addType ? 'Project' : 'Task'}</u>
              </strong>{' '}
              !
            </MDBModalTitle>
          </MDBModalHeader>
          ...
          <MDBModalFooter>
            <MDBBtn
              onClick={() => tryAdd(reduxDispatch, state, idProject)}
              color={addType ? 'info' : 'success'}
            >
