...
import {
  removeHugeTasks,
  updateHugeTask,
} from '../../../redux/slices/HugeTasksSlice';
...
const KanbanPiece = ({
  ...
  const isAddVisible =
    scopeSelected === SelectedScopeEnum.PROJECT && togglePopup !== undefined;

  return (
    <Fragment>
      <MDBRow
        className={
          'justify-content-center w-100 py-2 m-0 shadow rounded-top align-items-center ' +
          color
        }
      >
          ...
          return (
            <div key={key} className="card my-1 mx-2 border shadow-lg">
              <div className="col card-body">
                <h5
                  onClick={() => onClickKanbanPiece(dispatch, task.id!)}
                ...
                <div className="d-flex justify-content-between">
                  {task.state !== 'not started' && (
                    <MDBIcon
                      onClick={() =>
                        dispatch(updateHugeTask(setPrevState(task)))
                      }
                  ...
                  {task.state !== 'done' && (
                    <MDBIcon
                      onClick={() =>
                        dispatch(updateHugeTask(setNextState(task)))
                      }
                      style={{ cursor: 'pointer' }}
                      icon="arrow-right"
                    />
              ...
              <div className="card-footer d-flex justify-content-end">
                <MDBBtn
                  onClick={() => dispatch(removeHugeTasks(task))}
                  className="btn-close"
                  color="none"
                />
              </div>
