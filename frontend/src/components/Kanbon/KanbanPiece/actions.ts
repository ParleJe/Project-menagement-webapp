...
const setNextState = (task: Simplified): Simplified => {
  switch (task.state) {
    case ExecutionStatusEnum.NOT_STARTED:
      return setTaskState(task, ExecutionStatusEnum.IN_PROGRESS);
    case ExecutionStatusEnum.IN_PROGRESS:
      return setTaskState(task, ExecutionStatusEnum.DONE);
    default:
      return task;
  }
};
...
const setPrevState = (task: Simplified): Simplified => {
  switch (task.state) {
    case ExecutionStatusEnum.DONE:
      return setTaskState(task, ExecutionStatusEnum.IN_PROGRESS);
    case ExecutionStatusEnum.IN_PROGRESS:
      return setTaskState(task, ExecutionStatusEnum.NOT_STARTED);
    default:
      return task;
  }
};
