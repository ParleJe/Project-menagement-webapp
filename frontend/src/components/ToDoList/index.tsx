  ...
  return (
    <Fragment>
      <MDBInputGroup className="mb-2 shadow">
        ...
        />
        <MDBBtn
          onClick={() => dispatchAddToDo(name, selected, dispatch)}
          outline
          color="primary"
        >
        ...
        <MDBListGroup className="m-0 p-0 px-2">
          {ToDos.map((el: Simplified, key: number) => {
            return (
              <MDBListGroupItem
                key={key}
                color={key % 2 === 0 ? 'dark' : 'light'}
              >
                <div className="d-flex w-100 justify-content-between ">
                  <h5>{el.name}</h5>
                  <div className="d-flex">
                    <MDBSwitch
                      checked={el.state === ExecutionStatusEnum.DONE}
                      onClick={() => dispatchUpdateToDo(el, dispatch)}
                    />
                    <MDBBtn
                      onClick={() => dispatchRemoveToDo(el.id!, dispatch)}
                      className="btn-close"
                      color="none"
                    />
                  </div>
