...
const dispatchAddToDo = (name: string, idTask: number, reduxDispatch: Dispatch<any>) => {
    const requestPayload = createAddToDoPayload(name, idTask);

    return reduxDispatch(addToDo(requestPayload));
};

const dispatchUpdateToDo = (toDo: Simplified, reduxDispatch: Dispatch<any>): void => {
    const updatedState = toDo.state === ExecutionStatusEnum.NOT_STARTED ? ExecutionStatusEnum.DONE : ExecutionStatusEnum.NOT_STARTED;
